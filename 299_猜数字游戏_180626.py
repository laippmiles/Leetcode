...
class Solution(object):
    def getHint(self, secret, guess):
        ...
        :rtype: str
        """
        sizeOfNum = len(secret)
        numOfA = 0
        for i in range(sizeOfNum):
            if secret[i] == guess[i]:
                numOfA += 1
                #先得到公牛个数
        numOfB = sum([min(secret.count(x), guess.count(x)) for x in '0123456789']) - numOfA
        #奶牛个数是（每个数字在两个字符串的较小个数）之和，再减去公牛数
        return str(numOfA) + 'A' + str(numOfB) + 'B'
        #构造字符串输出即可